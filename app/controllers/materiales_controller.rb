  ...
  private
    ...
    def materiale_params
      params.require(:materiale).permit(:item, :onyx_p_n, :type, :description, :p_n, :substitute_code, :substitute_p_n, :footprint, :mark,
       ...
       :acknowledgements_attributes => [:id, :acknowledgement, :_destroy], :materiale_manufacture_ships_attributes => [:id, :price, :delivery_time, :manufacture_id, :_destroy], 
       :materiale_pics_attributes => [:id, :materiale_pic, :_destroy])
    end
  end
